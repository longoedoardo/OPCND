MODULE TriangleQuadratureDunavant
...
USE TypesDef
  ...
  SUBROUTINE TriangleQuadratureDunavantPoints(rule, P_ref, W_ref)
    ...
    REAL(dp), ALLOCATABLE, INTENT(OUT) :: P_ref(:,:)
    REAL(dp), ALLOCATABLE, INTENT(OUT) :: W_ref(:)
    !*******************************************************************************
    ! Variabili locali
    !*******************************************************************************
    INTEGER :: order_num
    REAL(dp), ALLOCATABLE :: xyrif(:,:)
    REAL(dp), ALLOCATABLE :: wrif(:)
    !*******************************************************************************

    ! Calcolo indice della regola
    CALL dunavant_order_num(rule, order_num)

    ! Calcolo coordinate e pesi sul triangolo di riferimentos
    ALLOCATE(xyrif(2,order_num))
    ALLOCATE(wrif(order_num))
    CALL dunavant_rule(rule, order_num, xyrif, wrif)

    ! Matrice dei punti
    IF (ALLOCATED(P_ref)) DEALLOCATE(P_ref)
    ALLOCATE(P_ref(order_num,2))

    P_ref = TRANSPOSE(xyrif)

    ! Vettore dei pesi
    IF (ALLOCATED(W_ref)) DEALLOCATE(W_ref)
    ALLOCATE(W_ref(order_num))
...
END SUBROUTINE TriangleQuadratureDunavantPoints

SUBROUTINE shiftingTriangleQuadratureDunavant(V, nodi_rif, pesi_rif, P, W)
    ...
    !********************************************************************************
    ! INPUT:
    !   V        - (3 x 3) Coordinate dei vertici del triangolo fisico.
    !   nodi_rif - (Np x 2) Coordinate dei nodi sul triangolo di riferimento.
    !   pesi_rif - (Np) Pesi sul triangolo di riferimento.
    !
    ! OUTPUT:
    !   P        - (Np x 3) Coordinate dei punti di quadratura nel triangolo
    !              fisico.
    !   W        - (Np) Pesi di quadratura sul triangolo fisico.
    !
    !********************************************************************************
    ...
    ! al triangolo reale.
    DO i = 1, n_points
      P(i,:) = V(1,:) + nodi_rif(i,1)*A + nodi_rif(i,2)*B
    END DO

    ! Area del triangolo reale corrisponde alla metà del modulo del prodotto vettoriale A x B
    Area = 0.5_dp * SQRT( (A(2)*B(3) - A(3)*B(2))**2 &
                        + (A(3)*B(1) - A(1)*B(3))**2 &
                        + (A(1)*B(2) - A(2)*B(1))**2 )

    W = Area * pesi_rif

END SUBROUTINE shiftingTriangleQuadratureDunavant
...
END MODULE TriangleQuadratureDunavant