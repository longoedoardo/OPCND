PROGRAM mainLALO4D
    ...
    !**********************************************************************
    ! Variabili locali attive (Adattate per il 4D spazio-temporale)
    !**********************************************************************
    TYPE(t_polyhedron_4D)       :: poly_4D
    INTEGER                     :: k
    ...
    ! Avvia il cronometro CPU all'inizio delle operazioni
    CALL CPU_TIME(t_start)

    ade = 5

    WRITE(*,*) "Inizio caricamento geometria 4D..."
    ! Lettura corretta con i file vertex.dat (t=0), vertex_new.dat (t=1) e tri.dat
    CALL MeshReader4D('data/vertex.dat', 'data/vertex_new.dat', 'data/tri.dat', poly_4D)

    !**********************************************************************
    ! INIZIO PARTE SHAPE-INDEPENDENT (4D)
    !**********************************************************************

    WRITE(*,*) "Inizio parte shape-indipendent (4D)..."
    
    ! Griglia tensoriale Gauss-Chebyshev 4D: input ade, output XYZWT_tens_ref (Mx5)
    CALL cub_gausscheb_tens4D(ade, XYZWT_tens_ref)
    ...
    WRITE(*,*) "Fine parte shape-indipendent (4D)!"
    WRITE(*,*) "**********************************************************************"
 
    !**********************************************************************
    ! INIZIO PARTE SHAPE-DEPENDENT (4D)
    !**********************************************************************
    ...
    CALL chebyshev_moments_polyhedron_4D(poly_4D%vertici_4D, poly_4D%Hyperfacets, ade, chebyshev_indices, poly_4D%bbox_4D, moments_ch)

    ! Riscalamento della regola 4D dal cubo di riferimento al bbox 4D del poliedro
    CALL scale_rule(XYZWT_tens_ref, poly_4D%bbox_4D, XYZWT_tens)
    ...
    I_risultato = SUM(XYZWT(:, 5) * fXYZWT)

    ! Ferma il cronometro alla fine dei calcoli
    CALL CPU_TIME(t_end)

    WRITE(*,*) "**********************************************************************"
    WRITE(*, '("Integrale I_risultato 4D = ", ES22.15)') I_risultato
    WRITE(*, '("Tempo impiegato        = ", F10.5, " secondi")') (t_end - t_start)
    WRITE(*,*) "**********************************************************************"

    CONTAINS
 
    !**********************************************************************
    ! Funzione integranda f(x,y,z,tau) - Spaziotemporale 4D
    !**********************************************************************
