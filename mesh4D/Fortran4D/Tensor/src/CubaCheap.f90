MODULE CubaCheap
   ...
   SUBROUTINE chebyshev_moments_polyhedron_4D(vertici_4D, Hyperfacets, ade, chebyshev_indices, dbox, moments)
      ...
      num_facets  = SIZE(Hyperfacets)

      ALLOCATE(moments(num_moments))
      moments = 0.0_dp

      ! Baricentro globale usato da PrismQuad4D per orientare verso l'esterno
      DO ii = 1, 4
         baricentro_4D(ii) = SUM(vertici_4D(:, ii)) / REAL(SIZE(vertici_4D, 1), dp)
      END DO

      ! Regola di quadratura sul prisma di riferimento:
      ! triangolo 2D x intervallo temporale [0,1].
      CALL reference_prism_quadrature(ade, XI_ref, ETA_ref, T_ref, W_ref)
      ...
      DO k = 3, num_facets
         ...
         IF (ALLOCATED(V_prism)) DEALLOCATE(V_prism)
         ALLOCATE(V_prism(SIZE(nodes_id), 4))
         DO ii = 1, SIZE(nodes_id)
            V_prism(ii, :) = vertici_4D(nodes_id(ii), :)
         END DO

         CALL PrismQuad4D(V_prism, XI_ref, ETA_ref, T_ref, W_ref, baricentro_4D, XYZTW, WV_X)

         CALL cubature_tens_chebyshev_facet_4D(XYZTW, WV_X, ade, chebyshev_indices, dbox, facet_moments)

         moments = moments + facet_moments

         DEALLOCATE(V_prism, XYZTW, WV_X, facet_moments, nodes_id)

      END DO

      DEALLOCATE(XI_ref, ETA_ref, T_ref, W_ref)

   END SUBROUTINE chebyshev_moments_polyhedron_4D

   SUBROUTINE cubature_tens_chebyshev_facet_4D(XYZTW, WV_X, ade, chebyshev_indices, dbox, chebyshev_moms)
      ...
      ! Allocazione e scalatura dei punti sul dominio [-1, 1]^4
      ALLOCATE(X_ref(n, 4))
      DO c = 1, 4
      ...
      ! Integrazione analitica lungo la direzione X
