PROGRAM example_rigidmotion
   ...
   theta = PI / 4.0_dp
   rot = RESHAPE([ &
      COS(theta), -SIN(theta), 0.0_dp, &
      SIN(theta),  COS(theta), 0.0_dp, &
      0.0_dp,      0.0_dp,     1.0_dp], [3,3], ORDER=[2,1])
   translation = [0.20_dp, 0.10_dp, 0.15_dp]

   DO i = 1, 8
      vertices_final(i,:) = MATMUL(rot, vertices_initial(i,:)) + translation
   END DO
   ...
   WRITE(*,'(A)')   '       Cubatura su Cubo in Rototraslazione'
   WRITE(*,'(A,/)') '**************************************************************'
   ...
   CALL run_case(1, 'f_1(x,y,z,tau) = 1', 16.0_dp / 3.0_dp + 4.0_dp * SQRT(2.0_dp) / 3.0_dp, 1)
   CALL run_case(2, 'f_2(x,y,z,tau) = tau^2', 28.0_dp / 15.0_dp + 2.0_dp * SQRT(2.0_dp) / 5.0_dp, 2)
...
END PROGRAM example_rigidmotion
